...
namespace kodo
{
    ...
    struct backward_policy
    {
        ...
        backward_policy(const Stack& stack)
            : m_stack(stack),
              m_index(stack.symbols()),
              m_end(1)
        { }

        backward_policy(const Stack& stack, uint32_t start)
            : m_stack(stack),
              m_index(start),
              m_end(1)
        { }

        backward_policy(const Stack& stack, uint32_t start, uint32_t end)
            : m_stack(stack),
              m_index(start),
              m_end(end)
        { }
    ...
    public:
        ...
        void initialize(Factory& the_factory)
        {
            ...
            // from symbols to 0.
            m_maximum_pivot = the_policy::min(0, the_factory.symbols());
        }
